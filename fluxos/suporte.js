...
module.exports = function criarFluxoSuporte(ctx) {
    ...
    async function gerarDatasDisponiveis() {
        const datas = [];
        const hoje = new Date();
        ...
        const manhaHoje = await banco.agendamentos.verificarDisponibilidade(hojeBanco, 'manha');
        const tardeHoje = await banco.agendamentos.verificarDisponibilidade(hojeBanco, 'tarde');
        ...
        // =====================================================
        let diasTestados = 0;
        let diasAdicionados = datas.length;
    ...
    async function abrirChamadoComMotivo(deQuem, nome, motivo, extras = {}) {
        ...
        const chamadoId = await banco.dbAbrirChamado(deQuem, nome || null, motivo);
    ...
    async function iniciar(deQuem, msg, motivo = null) {
        ...
        if (motivo === 'troca_senha') {
            ...
            await banco.dbSalvarHistorico(deQuem, 'assistant', 'Iniciou fluxo troca de senha.');
    ...
    async function handle(deQuem, msg) {
        ...
        if (etapa === 'aguardando_agendamento_dia') {
            ...
            if (/^[1-5]$/.test(escolha)) {
                ...
                if (index >= 0 && index < datas.length) {
                    ...
                    const manha = await banco.agendamentos.verificarDisponibilidade(dataEscolhida.dataBanco, 'manha');
                    const tarde = await banco.agendamentos.verificarDisponibilidade(dataEscolhida.dataBanco, 'tarde');
        ...
        if (etapa === 'aguardando_agendamento_periodo') {
            ...
            if (opcaoEscolhida) {
                ...
                const disponibilidade = await banco.agendamentos.verificarDisponibilidade(dados.dataBanco, periodo);
                
                if (!disponibilidade.disponivel) {
                    const manha = await banco.agendamentos.verificarDisponibilidade(dados.dataBanco, 'manha');
                    const tarde = await banco.agendamentos.verificarDisponibilidade(dados.dataBanco, 'tarde');
                ...
                await banco.dbSalvarHistorico(deQuem, 'assistant', msgFinal);
